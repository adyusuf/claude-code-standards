...
- Detay ve edge tarafı: `20-sertlestirme.md`.
...
- Envanteri `SETUP.md`'de tutulur (`18-kurulum-ve-ortam.md`): ne, nereden alınır, nerede durur, sahibi kim, geçerlilik/rotasyon.
...
- Bot/otomasyon koruması edge katmanında (`19-cloudflare-ve-edge.md`).
...
| **A05** | Security Misconfiguration | `20-sertlestirme.md` tamamı: prod'da Swagger/debug kapalı, güvenlik başlıkları, varsayılan parola yok, dizin listeleme kapalı, CORS dar |
...
| **A09** | Security Logging & Monitoring Failures | `17-gozlemlenebilirlik.md`: güvenlik olayları log'lanır, audit log append-only, anormal desen alarmı, correlation id |
...
(over-posting → ayrı input DTO). Bunlar `07-api-tasarimi.md` kurallarıyla kapatılır.
...
- [ ] Yedek + restore provası güncel (`21-yedekleme-ve-kurtarma.md`)
