...
# tekrar ödenir. Bütçe `scripts/md-butce.tsv`'de (cırcır: tavan yalnız iner).
#
# Kullanım:
#   scripts/md-boyut-kapisi.sh              # denetle (kapı)
#   scripts/md-boyut-kapisi.sh --guncelle   # tavanları BUGÜNKÜ boyuta çek
#
...
if [ -n "${MD_BUTCE:-}" ]; then BUTCE="$MD_BUTCE"; else
  for aday in "$KOK/scripts/md-butce.tsv" "$KOK/md-butce.tsv" "$KOK/docs/md-butce.tsv"; do
...
if [ -z "${BUTCE:-}" ] || [ ! -f "$BUTCE" ]; then
  ...
  if [ "$HOOK" -eq 1 ]; then
    echo "· CLAUDE.md butcesi bu projede kurulu degil (md-butce.tsv yok)."
    echo "  Kurmak icin: /proje-standardi-uygula  ya da"
    echo "  cp ~/.claude/scripts/md-*.sh ~/.claude/scripts/md-*.py scripts/ && bash scripts/md-boyut-kapisi.sh --guncelle"
    exit 0
  fi
...
while IFS= read -r f; do
  rel="${f#$KOK/}"
  case "$rel" in .claude/*|**/node_modules/*|docs/claude-md-arsiv/*) continue ;; esac
...
Yapılacak (sırayla):
  1. Eklediğin şey bir KURAL mı, yoksa gerekçe/tarihçe/test tutanağı mı?
     Gerekçe ve tarihçe aktif dosyada DURMAZ — docs/<ayak>-kararlar.md'ye taşı,
     aktif dosyada kural cümlesi + detay linki kalsın.
  2. Gerçekten aktif dosyada kalması gereken yeni bir kuralsa:
     scripts/md-butce.tsv'de tavanı yükselt ve NOT sütununa GEREKÇE yaz.
  3. Sadeleştirme yaptıysan tavanı indir:  scripts/md-boyut-kapisi.sh --guncelle
SON
  exit 1
fi
