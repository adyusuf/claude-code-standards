...
| `measurement-ledger.py` | Generates `docs/measurement-ledger.tsv`: **one row per session or agent run** (UTC start, role, model, the four token counters, list-price USD, the configuration-cut ordinal, the SDLC steps it ran and **how many seconds each took**, the **project nickname**, and the **end time**). Every column is derived from the transcripts — **no hand-filled column**. Rows are **merged by id and never dropped** (Claude Code deletes old transcripts, the ledger keeps them); no project name, path or command is written, and every field must match a strict pattern or the whole write is refused. Flags: `--write`, `--days N` (default 30), `--ledger PATH` (another file, for a trial run); ~35 s for a full 30-day pass. **`--auto [--detach] [--min-interval S]`** is the incremental mode the Stop hook runs: it reads only transcripts modified since the last write, skips when the last write is under S seconds old (default 600), holds a lock, replaces the file atomically, never raises, and with `--detach` returns to the caller at once. It writes the **git-ignored** `docs/measurement-ledger.tsv` of the repository the script *really* lives in (a symlink is resolved). The file stays local: a hook that rewrites a tracked file leaves that worktree dirty and blocks any promotion that touches it. Back it up if the history matters; Claude Code deletes old transcripts, and this file is then the only copy of those rows. Tests: `python3 -m unittest scripts/tests/test_measurement_ledger.py scripts/tests/test_ledger_auto.py`. |
...
| `measurement-report.py` | Writes four local, git-ignored reports beside the ledger, each **grouped by day**: `measurement-daily-by-project.md` (nicknames), `-by-role.md`, `-by-kind.md`, `-by-model.md`. A row is charged WHOLE to the local day its session **ended** (the last record of the transcript), or to the day it started when no end was recorded (marked ≈). `measurement-ledger.py` rewrites all four whenever it writes the ledger. Project nicknames come from `project-nicknames.tsv` (local: it holds real folder keys); an unmapped project shows as `unmapped-xxxxxx`, and a nickname that contains a real project name refuses the write. Tests: `python3 -m unittest scripts/tests/test_measurement_report.py`. |
