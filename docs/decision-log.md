...
> `standards/00-working-method.md` §6a gereği: aktif `CLAUDE.md` bir **kural
...
> `scripts/md-rule-gate.py` ile denetlendi. Aktif dosyadaki kural cümlesi
...
## §28 — Denetçi her geçişte eksik/yanlış KONTROL EDER, GERİ GÖNDERİR ve DÜZELTTİRİR; mal
    ...
    - **Maliyet tablosu ölçülerek düzeltilir.** `role-selection.md` §8'deki model kırılımı **tahmindir** (ölçülen tek sayı: tur başına $6,9). Her gerçek ajan turunda bildirimdeki `subagent_tokens` × model fiyatı hesaplanır, alt-üst sınır olarak §8'deki **ölçüm defterine** yazılır. Tablo **tek ölçümle değiştirilmez** — en az 3 kayıt ya da gerçek bir uçtan uca C/D turu gerekir.
    - **Üç denetim boşluğu kapatıldı:** `analyst` bulgusunu üreten **komutu** da döndürür (yoksa bulgu "doğrulanmadı" sayılır) · `devops` çıktısı **`qa`'ya girer** ("config, kod değil" atlama gerekçesi değildir) · `product-manager` çıktısı zincire otomatik akmaz, **kullanıcı onayına** gider.
    **Why:** Dokuz rol ajanından yalnız `developer` gerçek anlamda denetleniyordu (`qa` → ben). `analyst` yanlış sayarsa, `devops` kapıyı bozarsa, `product-manager` kapsamı kaçırırsa hata sessizce aşağı akıyordu; `qa`'ya ikinci denetçi ajan eklemek ise subagent token'ı ~4x pahalı olduğu için karşılığını vermiyordu. **How to apply:** Ayrıntı ve rakamlar `~/.claude/modes/role-selection.md` §2a, §3, §5, §6, §7, §8'de. Mod A'da ajan yoktur; eksik kontrolü orada #24 ile aynı işi görür.
...
- ❌ `product-manager` çıktısını kullanıcı onayı almadan zincire akıtmak → #28
- ❌ Maliyeti yalnız tur sonunda söylemek; devir satırlarını atlamak → #28
- ❌ `analyst` bulgusunu doğrulama komutu olmadan kesinmiş gibi kullanmak → #28
...
## Görev → hangi standardı oku
| Görev | Oku |
| Yeni özellik / kapsam / kabul kriteri | `standards/01-product-design.md` |
| Ekran, bileşen, stil, erişilebilirlik | `standards/02-ui-ux.md` |
| Genel kod yazımı, isimlendirme, hata yönetimi | `standards/03-coding-general.md` |
| .NET endpoint, servis, EF Core, DI | `standards/04-dotnet.md` |
| React sayfa/bileşen/state/veri çekme | `standards/05-react.md` |
| React Native / Expo ekran, store yayını | `standards/06-mobile.md` |
| Endpoint sözleşmesi, hata formatı, sayfalama | `standards/07-api-design.md` |
| Şema, migration, index, transaction | `standards/09-database.md` |
...
| Auth, OWASP, sır yönetimi, KVKK | `standards/15-security.md` |
| Yavaşlık, cache, bundle, Core Web Vitals | `standards/16-performance.md` |
| Log, metrik, trace, alarm, incident | `standards/17-observability.md` |
| **Kurulum, ön koşullar, sır/token envanteri, `.env`** | `standards/18-setup-and-environment.md` |
| **Cloudflare: DNS, TLS, WAF, cache, Tunnel, Workers, R2** | `standards/19-cloudflare-and-edge.md` |
| **Sunucu/uygulama sertleştirme, güvenlik başlıkları, IIS/Docker** | `standards/20-hardening.md` |
| **Yedekleme, restore provası, RPO/RTO, felaket kurtarma** | `standards/21-backup-and-recovery.md` |
Şablonlar: `standards/templates/` — proje CLAUDE.md, **SETUP.md**, PR, ADR, user story.
...
28. **Denetçi her geçişte eksik/yanlış KONTROL EDER, GERİ GÖNDERİR ve DÜZELTTİRİR; maliyet her devirde hatırlatılır (2026-09-07, kullanıcı kararı, tüm projeler için geçerli).** Ajan kullanılan her turda:
    - **Eksik kontrolü bloğu zorunlu.** Çıktısına başkasının güveneceği her rol (`qa`, `analyst`, `devops`, `test-writer`, `product-manager`) ve **her devirde orkestratör**, raporunu **kanıt bloğuyla** kapatır: *Doğrulama (koşulan komut/okunan aralık) · Madde eşlemesi (istenen her madde → dosya:satır) · Kapsanmayan → Sonuç: ciddi eksik YOK | VAR.* Blok temiz geçilse bile yazılır — görünmeyen kontrol yapılmamış kontroldür. ⚠️ **Bu bir soru değil, kontroldür:** denetçi "eksik var mı?" diye kullanıcıya da üreten role de **sormaz**; bakar, kanıtı yazar, kararı kendi verir. **Doğrulanamayan şey "tamam" sayılmaz.**
    - **Eksik varsa GERİ GÖNDERİLİR, kullanıcıya taşınmaz.** İş devretmez, üretene döner (`qa` → `developer`/ben → yeniden `qa`); geri gönderme **ne eksik · hangi kanıtla · ne yapılacak** taşır ve kontrol baştan başlar. Kullanıcıya gitmesi yalnız §5 durma sebeplerinde ve tavanda olur — o da **soru değil, durum raporu**.
...
29. **Test kapsamı her kod tabanında EN AZ %80 (satır) — istisnasız (13/09/2026, kullanıcı kararı: "irademdir", tüm projeler için).**
    ...
    - **Sahte kapsam yasak:** E2E bu sayıya girmez (ayrı ölçü). Kapsamı artırmak için assert'sız ya da hiçbir şey yakalamayan test yazmak eşiği gevşetmekle aynıdır — eklenen test mutasyonla bir hatayı yakalamalı (`standards/10-test-strategy.md` §7).
...
| Alan/endpoint değiştirme, deprecation | `standards/08-backward-compatibility.md` |
| Unit/integration test yazımı | `standards/10-test-strategy.md` |
...
- Detay: `standards/00-working-method.md`
...
Ölçüm yöntemi ve tekrarı: `scripts/prefix-measure.py`, `standards/00` §6b.

- **§21-23 (madde 3):** X/Y/Z takım modları `modes/archive/`'e alındı — bugün
  ...
  yükleniyordu. Geri getirme koşulu `modes/archive/README.md`'de.
- **§28 (madde 4):** 14 ajan dosyasındaki "Eksik kontrolü" metni birebir
  kopyaydı (toplam ~24 KB); tek sürüme indi, tam kural `role-selection.md` §7'de.
