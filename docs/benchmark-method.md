...
| When a run ended | the timestamp of the transcript's last record | ledger column `ended`; recorded as information; the day reports charge a run to the local day it **started** |
...
The per-day reports (`measurement-daily-by-project|role|kind|model.md`) are generated from the ledger, charge a run whole to the day its session **started**, and are equally local. The ledger is a local, git-ignored file; back it up if the history matters. `modes/role-selection.md` §8 also holds a hand-written ledger of early `qa` turns.
