...
**C'den gelen dokuz:** `urun-yoneticisi` · `analiz` · `mimar` · `tasarimci` ·
`gelistirici` · `test-yazar` · `qa` · `devops` · `belge`

**D'de açılan beş:** `guvenlik` · `veri` · `kapsam-denetcisi` · `e2e-yazar` ·
`gozlemlenebilirlik`

⚠️ **D, C'nin üst kümesidir** (B ⊂ C ⊂ D). Beş yeni rolün tamamı **denetçidir**:
hiçbiri ürün kodu yazmaz, ikisi (`veri`, `e2e-yazar`) kendi dar alanında üretir.
...
|---|---|
| `guvenlik` | #19 — SAST, sır taraması, CVE, ZAP, OWASP Top 10 eşlemesi |
| `veri` | #4 + #9 — additive şema evrimi, migration, index, transaction |
| `kapsam-denetcisi` | #29 — her kod tabanında %80 satır kapsamı, payda dürüstlüğü |
| `e2e-yazar` | #30 + #31 + #32 — e2e verisi, koşum döngüsü, sınıflandırma |
| `gozlemlenebilirlik` | #17 standardı — log, metrik, trace, alarm, performans |
...
- `gelistirici` yalnız **izole, sözleşmesi net** parçalarda; çapraz-katman iş bende.
...
| Üretici | Denetçisi |
|---|---|
| `gelistirici` · `test-yazar` · `devops` | `qa` |
| `veri` · `e2e-yazar` · `gozlemlenebilirlik` | `qa` |
| **`guvenlik`** | **orkestratör** (doğrudan) |
| **`kapsam-denetcisi`** | **orkestratör** (doğrudan) |
| `qa` | orkestratör — kritik bulguları ben doğrularım |
| `urun-yoneticisi` | **kullanıcı** (§2a kapsam kapısı) |
| `mimar` · `tasarimci` | orkestratör (§7 muafiyeti) |

⚠️ **`guvenlik` ve `kapsam-denetcisi` neden `qa`'ya girmez:** C'de `qa`'ya üç
...
nihaidir** — `qa`'nın güvenlik ekseni `guvenlik`'te zaten daha derin işleniyor,
...
⚠️ **`mimar` muafiyeti D'de daha riskli:** planı artık `veri`, `guvenlik` ve
`e2e-yazar`'ı da besliyor — etki alanı C'ye göre üç kat. Muafiyet sürüyor ama
plan sapması çıktığında zincir **bana** döner, `mimar`'a değil.
...
`veri` · `gozlemlenebilirlik` bu kısıtın **dışındadır** — çünkü bunlar bir
promosyon kapısının eşi değil, **kod yazılırken** gereken denetimlerdir:

- ⚠️ **`veri` özellikle `dev` ÖNCESİ koşar.** Migration `dev`'e girdikten sonra
  ...
  `veri`'siz `dev`'e merge edilmez.
- `gozlemlenebilirlik` de kod yazılırken değer üretir — eksik log'u üretimde
  fark etmek, tanım gereği geç kalmaktır.

`e2e-yazar` zaten ortam-bağlıdır: #31 gereği **yalnız `test`'e çıkmış kodla**
...
- Şema değişikliği içeren özellik — `veri` olmadan geriye uyumluluğu doğrulayan yok
