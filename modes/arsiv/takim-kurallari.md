...
> şey ajanın **ömrü ve iletişimi**. Rol seçimi hâlâ [`rol-secimi.md`](rol-secimi.md)
...
bildirir. Gerekçe: `rol-secimi.md`'nin "rol seçimi tek yerdedir" değişmezi ve
...
⚠️ Gerekçe: aksi hâlde `gelistirici` teammate'i sırası gelince `qa` görevini
kapıp **kendi kodunu kendi review eder** — C/Y'nin "üreten kendi işini
denetlemez" ilkesi sessizce çöker.

## 4. Denetim — `rol-secimi.md` §7 ile AYNI, taşıyıcısı farklı
...
`gelistirici`'ye gönderip aralarında kapatırsa §7'nin "orkestratör bloğu yutmaz"
...
- ⚠️ Liste, `rol-secimi.md` §6'daki tur başı/sonu raporunun **yerine geçmez** —
...
**Ölçüm:** `python3 ~/.claude/scripts/oturum-maliyeti.py <oturum-id>` — tahmin
değil, transcript'ten ölçer. İlk gerçek X/Y/Z turunda koşulur ve
`rol-secimi.md` §8 ölçüm defterine yazılır.
