...
⚠️ C, **B'nin üst kümesidir** — B'de açık olan `test-yazar` burada da açıktır.
Test yazımı `gelistirici`ye bırakılmaz: ürün kodu yazan ajanın kendi testini
...
- `gelistirici` ajanı yalnız **izole, sözleşmesi net** parçalarda kullanılır
...
- Ajan çağrısı öncesi sormam; **her devirde** maliyet satırı geçerim, tur
  sonunda toplarım (`rol-secimi.md` §6, §8). Eşik iki kademeli:
...
- Denetçi roller **sormaz, kontrol eder**: eksik/yanlış varsa iş üretene
  ...
  **bildirilir**.
- `urun-yoneticisi` çıktısı **senin onayına** gider (§2a); `devops` çıktısı
  `qa`'ya girer (§3).
...
C yetmez → [`D-genis-takim.md`](D-genis-takim.md) (14 rol).
