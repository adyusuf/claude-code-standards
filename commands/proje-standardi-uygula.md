...
- Yoksa: `~/.claude/standards/sablonlar/proje-claude-md.md` şablonundan, **gerçek** bilgilerle doldurarak oluştur.
...
`~/.claude/standards/sablonlar/setup-md.md` şablonundan — kritik bölüm **§3 sır/token envanteri**:
...
bash scripts/md-boyut-kapisi.sh --guncelle   # tavan = BUGÜNKÜ boyut
```
...
- Projenin merge/CI kapısına `bash scripts/md-boyut-kapisi.sh` adımını ekle.
...
- Zaten bir boyut kapısı varsa **ikincisini kurma**; var olanı kullan. **Ölçüt
  (tahmin etme, koştur):** `grep -rlE 'md-boyut-kapisi|md-butce|claude-md-budget' scripts/ .github/ 2>/dev/null`
...
değil) yapılır; sonucu `md-kural-kapisi.py` ile doğrulanır.

## 6. Çalışma modu (opsiyonel)

Proje `.claude/mode` dosyası taşımıyorsa mod **A** (ajan yok) geçerlidir.
Ajan isteniyorsa `/calisma-modu <B|C|D>`.
